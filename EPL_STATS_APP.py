...
try:
    ...
    if "Prénom" in df_classement.columns:
        group_cols.append("Prénom")
    if "Matricule" in df_classement.columns:
        group_cols = ["Matricule"] + group_cols
    if "Parcours" in df_classement.columns:
        group_cols.append("Parcours")
    if "Année_étude" in df_classement.columns:
        group_cols.append("Année_étude")
    if "sexe" in df_classement.columns:
        group_cols.append("sexe")

    if type_classement == "Moyenne générale":
        classement = (
            df_classement.groupby(group_cols, as_index=False)
            .agg(Moyenne=("Note", "mean"))
...
df_moyenne = df_modif.groupby(group_cols, as_index=False).agg(Moyenne=("Note", "mean"))
...
if not df_difficulte.empty:
    st.dataframe(df_difficulte, use_container_width=True)
else:
...
if df_pivot.shape[1] >= 2:
    ...
    st.warning("Pas assez de matières pour une corrélation.")
